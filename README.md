...
The release zip is written to `dist/QualityFullMonty_0.1.0.zip`.
...
First-time publishing needs a Factorio account API key from `https://factorio.com/profile` with `ModPortal: Publish Mods` permission. The first upload uses the Mod Publish API `init_publish` endpoint, then uploads `dist/QualityFullMonty_0.1.0.zip` to the returned upload URL.
